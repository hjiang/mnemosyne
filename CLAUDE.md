...
- **Retry while making progress**: the orchestrator reconnects and retries `syncFolder` on connection-level failures (signaled via the `connError` sentinel) as long as any of `NewLocations`, `NewEnvelopes`, or `NewDeletions` advanced during the attempt. `NewDeletions` counts messages removed by a completed `EXPUNGE`. When no progress is made the loop gives up and immediately stops processing all remaining folders — since one connection is shared, a dead connection makes further folders pointless. Envelopes already fetched are carried across retries (the in/out `envelopes` slice on `syncFolder`) so we don't refetch them.
- **Retention sweep robustness**: deletion of previously-backed-up messages runs in chunks aligned with `expungeBatchSize` (default 50). Each cycle is one `FilterBackedUpUIDs` SQL filter + one `MarkDeleted` STORE (batched UID set, not per-UID) + one `EXPUNGE` + one `last_swept_uid` write to `imap_folders`. After a mid-sweep connection drop, the cursor lets the next attempt skip UIDs whose mark+expunge already completed. The cursor resets to 0 on full sweep completion and on UIDVALIDITY change. Retention-sweep errors are classified by `isTransient` — only network-level failures wrap into `connError` (and trigger reconnect/retry); IMAP `*goiap.Error` (NO/BAD) propagates as a plain error so the retry loop moves on instead of burning the no-progress budget.
