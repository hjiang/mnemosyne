...
- **Wave A robustness**: deletion of previously-backed-up messages runs in chunks aligned with `expungeBatchSize` (default 50). Each cycle is one `FilterBackedUpUIDs` SQL filter + one `MarkDeleted` STORE (batched UID set, not per-UID) + one `EXPUNGE` + one `wave_a_cursor` write to `imap_folders`. After a mid-sweep connection drop, the cursor lets the next attempt skip UIDs whose mark+expunge already completed. The cursor resets to 0 on full sweep completion and on UIDVALIDITY change. Wave A errors are classified by `isTransient` — only network-level failures wrap into `connError` (and trigger reconnect/retry); IMAP `*goiap.Error` (NO/BAD) propagates as a plain error so the retry loop moves on instead of burning the no-progress budget.
