...
- **Retry while making progress**: the orchestrator reconnects and retries `syncFolder` on connection-level failures (signaled via the `connError` sentinel) as long as any of `NewLocations`, `NewEnvelopes`, or `NewDeletions` advanced during the attempt. `NewDeletions` counts messages removed by a completed `EXPUNGE` (not `MarkDeleted` calls, which are idempotent and would inflate the counter on every retry). Wave A (deletion of previously-backed-up messages) participates in this retry loop: a mid-deletion drop returns `*connError` so the loop can reconnect and resume. When no progress is made the loop gives up and immediately stops processing all remaining folders — since one connection is shared, a dead connection makes further folders pointless. Envelopes already fetched are carried across retries (the in/out `envelopes` slice on `syncFolder`) so we don't refetch them.
